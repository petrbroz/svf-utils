...
const { Svf2Client, Svf2ManifestHelper, Svf2ViewHelper } = require('..');
...
async function run(urn, outputDir) {
    try {
        ...
        const otgClient = new Svf2Client({ token: auth.access_token });
        ...
        for (const viewable of viewables) {
            const otgManifestHelper = new Svf2ManifestHelper(viewable.otg_manifest);
            ...
            for (const asset of otgManifestHelper.listSharedDatabaseAssets()) {
                ...
                const assetData = await otgClient.getAsset(urn, encodeURIComponent(asset.resolvedUrn));
            ...
            for (const view of otgManifestHelper.listViews()) {
                ...
                const viewData = await otgClient.getAsset(urn, encodeURIComponent(view.resolvedUrn));
                writeBuffer(viewPath, viewData);
                const otgViewHelper = new Svf2ViewHelper(JSON.parse(viewData.toString()), view.resolvedUrn);
